...
const parser = (tokens) => {
  ...
  function parseValue() {
    ...
    if (token.type === "identifier") {
      const identifierToken = token; // save identifier
      ...
      if (
        ...
        if (
          ...
          throw new Error("Expected closing bracket in array access");
        }
        current++; // move past ']'
        return {
          type: "AccessExpression",
          name: identifierToken.value, // use saved value
          index: index,
        };
      }
  ...
  function parseAssignment() {
    ...
    if (
      ...
      if (left.type !== "identifier" && left.type !== "AccessExpression") {
  ...
  function parseVariableDeclaration() {
    ...
    current++; // Move past type (or after [] if it existed)
...
const codeGen = (node) => {
  switch (node.type) {
    ...
    case "BinaryExpression":
      const leftExpr =
        node.left.type === "identifier" ? node.left.value : codeGen(node.left);
      const rightExpr =
    ...
    case "AssignmentExpression":
      ...
      if (node.isArrayElement) {
        return `${node.name}[${codeGen(node.index)}] = ${assignValue};`;
      }
    ...
    case "UnaryExpression":
      ...
      if (node.operator === "++") {
        return node.prefix ? `(${argName} = ${argName} + 1)` : `(${argName}++)`;
